...
const censorPdf: ProcessorFn = async (files, onProgress, options) => {
  ...
  for (let i = 0; i < pdfJsDoc.numPages; i++) {
    ...
    textContent.items.forEach((item: unknown) => {
      ...
      if (textItem.str && textItem.str.toLowerCase().includes(searchText.toLowerCase())) {
        const tx = textItem.transform!;
        pages[i].drawRectangle({
          x: tx[4] - 2, y: tx[5] - 2,
          width: (textItem.width || 0) + 4, height: (textItem.height || 0) + 4,
          color: rgb(0, 0, 0),
        });
      }
    });
...
const pdfToExcel: ProcessorFn = async (files, onProgress) => {
  ...
  for (let i = 0; i < doc.numPages; i++) {
    ...
    const itemsByY: Record<number, unknown[]> = {};
    content.items.forEach((item: unknown) => {
      const textItem = item as { transform: number[] };
    ...
    for (const y of sortedYs) {
      const items = itemsByY[y].sort((a: unknown, b: unknown) => (a as { transform: number[] }).transform[4] - (b as { transform: number[] }).transform[4]);
      allRows.push(items.map((item: unknown) => (item as { str: string }).str));
    }
    onProgress(10 + (i / doc.numPages) * 60);
  }
